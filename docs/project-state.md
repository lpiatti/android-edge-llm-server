...
- Phase 4 (Milestone 4.3): Ristrutturazione UI & System File Picker. Refactor layouts into organized tabs (Model Engine, Server Daemon, Test Suite), add an always-visible status island (Active Model, Server Address/State), make console logs collapsible to optimize vertical space, enforce execution state interlocks, and replace directory scanning with a native System File Picker.
